# tests/test_phase14_autonomous_agent.py
...
def mock_agent_dependencies():
    ...
    # The Agent's dependencies need to have an `llm_provider` attribute
...
# --- Evaluation for Task 14.1 & 14.2: Planning and Execution Logic ---
...
def test_agent_creates_test_aware_plan(mock_agent_dependencies):
    ...
    mock_idea_synth = mock_agent_dependencies["idea_synth"] # Get the IdeaSynthesizer mock
    
    # Mock the LLM to return a plan that includes a 'generate tests' step by mocking generate_ideas
    ...
    assert len(plan) == 2, "The plan should have two steps."
    assert "generate tests" in plan[1], "The generated plan should include a step for generating tests."

# --- Evaluation for Task 14.3: Self-Correction Logic ---
...
def test_agent_self_correction_prompting(mock_agent_dependencies):
    ...
    # The agent's attempt_fix method uses the code_generator's LLM provider.
    mock_code_gen = mock_agent_dependencies["code_generator"] # Get the CodeGenerator mock
